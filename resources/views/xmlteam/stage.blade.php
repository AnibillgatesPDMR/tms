...
?>
    <div class="row">
        ...
        <div class="col-md-1 page-action text-right">
        	<a href="{{ asset('xml/myjobs/list') }}" style="margin-right: 10px;" class="btn btn-danger btn-sm"> <i class="fa fa-arrow-left"></i> Back</a>
        </div>
    </div>
        ...
        <table class="table">
            ...
            @foreach($articleStage as $item)
                <tr>
                    ...
                    <td><a class="btn btn-xs btn-success" href="{{ asset('xml/file/list') }}/{{ last(request()->segments()) }}/{{$item->id}}">View</a></td>
                </tr>
            <?php $i++; ?>
            @endforeach
            </tbody>
