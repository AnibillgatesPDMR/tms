...
?>
    <div class="row">
        ...
        <div class="col-md-1 page-action text-right">
        	<a href="{{ asset('copy/jobs/list') }}" style="margin-right: 10px;" class="btn btn-danger btn-sm"> <i class="fa fa-arrow-left"></i> Back</a>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group @if ($errors->has('duedate')) has-error @endif">
                        ...
                        <div class="input-group col-md-12 date">
                            <input type="text" name="duedate" class="form-control pull-right" id="copyduedate" required="required">
                ...
                <div class="form-group @if ($errors->has('duedate')) has-error @endif">
                        ...
                        <div class="input-group col-md-12 date">
                            ...
                            <select name="jobtype" class="form-control" id="jobtype" required='required'>
                            <option value="">--Select--</option>
                            @foreach($jobtype as $jobs)
                            <option value="{{$jobs->id}}" style="color:{{$jobs->color_code}};">{{$jobs->job_type}}</option>
                            @endforeach
                        </select>                            
                        </div>
                        @if ($errors->has('jobtype')) <p class="help-block">{{ $errors->first('jobtype') }}</p> @endif
                    </div>


           <div class="form-group @if ($errors->has('duedate')) has-error @endif">
                        {!! Form::label('queries', 'Queires') !!}
                        <div class="input-group col-md-12 date">

                           <textarea cols="72" id="queries" name="queries"></textarea> 
                           
                                                     
                        </div>
                        @if ($errors->has('queries')) <p class="help-block">{{ $errors->first('queries') }}</p> @endif
                    </div>
