...
class Role extends \Spatie\Permission\Models\Role
{
    ...
    public static function common_emails($email_array)
    {
        ...
        $team_name = 'TMS';
        $from_email = 'tmscompuscript@gmail.com';
        $to_email = $email_array['datas']['data-manager'];  
        // $to_email = 'venkat@pdmrindia.com'; 
        //$cc_email = 't.nolan@compuscript.com';

          $cc_email5 = 't.nolan@compuscript.com';
          $cc_email = 'm.lyons@compuscript.com';
    //    $cc_email5 = 'gomathi@pdmrindia.com';
    //    $cc_email = 'venkat@pdmrindia.com';
        ...
        if(strcmp($email_array['Subject'],"Compuscript Forget Password")==0) {
                ...
                try {
                    \Mail::send($email_template, $data, function ($message) use ($Subject, $team_name, $from_email, $to_email, $cc_email) {
                       ...
                       // $message->cc($cc_email, $cc_email);
                        $message->from($from_email, $team_name);
                    });
                    return "Success";
                } catch (Exception $ex) {
                    dd($ex->getMessage());
        ...
        if(strcmp($email_array['Subject'],"Leave request")==0) {
                    try {
                        \Mail::send($email_template, $data, function ($message) use ($Subject, $team_name, $from_email, $to_email, $cc_email,$cc_email5) {

                            $message->to($to_email, $to_email)->subject($Subject);
                            $message->bcc($cc_email, $cc_email);
                            $message->bcc($cc_email5, $cc_email5);
                            $message->from($from_email, $team_name);
        ...
        if(strcmp($email_array['Subject'],"Your leave request is approved")==0) {
        try {
            \Mail::send($email_template, $data, function ($message) use ($Subject, $team_name, $from_email, $to_email, $cc_email) {
    ...
    if(strcmp($email_array['Subject'],"Your leave request is not approved")==0) {
        try {
            \Mail::send($email_template, $data, function ($message) use ($Subject, $team_name, $from_email, $to_email, $cc_email) {
    ...
    if(strcmp($email_array['Subject'],"Forgot login")==0) {

        if($to_email =="m.lyons@compuscript.com"){
            $to_email = 't.nolan@compuscript.com';
        }
    ...
    public static function getAll_forgetlogin_currentMth($user_id,$role,$group_email)
    {
      ...
      //  $query->where('user_forgetlogin.forget_login_status!=', 'Approved');
        ...
        if($user_id!='1') {
         ...
         $query->orderBy('user_forgetlogin.id', 'desc');
    ...
    public static function getAll_Leaves($user_id,$role,$group_email)
    {
       ...
       if($role=='17') {
          ...
          //  $leave_Details_query->where('user_leavelist.created_date', $current_year);
            $leave_Details_query->where('user_leavelist.created_date', 'like', '%' .$current_year. '%' );
  ...
  // $leave_Details_query->where('user_leavelist.from_date', $current_year);
   
    //$leave_Details_query->where(DB::raw('YEAR(created_date)=2021'));
...
exit;  */
    ...
    public static function leavebalance_count($user_id,$leave_type) {
        ...
        ->select(DB::raw("SUM(no_ofdays) as totalleave"))             
        ->where('user_id', '=', $user_id)
        ->where('leave_type', '=', $leave_type)
        ->where('leave_status', '=', 'Approved')
        //->whereYear('created_date', '=', $current_year)
        //->whereYear('to_date', '=', $current_year)
...
public static function forget_approval_mgr_bulk($fid) {
    
    
    $status = 'Approved';

    return DB::table('user_forgetlogin')->where('id', $fid)->update(['forget_login_status' => $status]);
}

public static function forget_reject_mgr_bulk($fid) {
...
public static function getAll_EmployeeLeave_All($emp_id)
  ...
  public static function get_forget_applied_check($logged_in_at, $user_id)
    ...
    public static function ticket_emails($email_array)
    {
        ...
        $from_email = 'tmscompuscript@gmail.com';
        $to_email =  'venkat@pdmrindia.com'; //$email_array['datas']['data-manager'];  // 'muhammed@pdmrindia.com'; 
       // $cc_email = 'tmscompuscript@gmail.com';
        //$cc_email = 'selvi@pdmrindia.com';
        $cc_email = 't.nolan@compuscript.com';
        $cc_email_1 = 'aravind@pdmrindia.com';
       // $cc_email = 'prince@pdmrindia.com';
        $email_template = $email_array['template_name'];
    
             try {
                    \Mail::send($email_template, $data, function ($message) use ($Subject, $team_name, $from_email, $to_email, $cc_email,$cc_email_1) {

                        $message->to($to_email, $to_email)->subject($Subject);
                        $message->cc($cc_email, $cc_email);
                        $message->cc($cc_email_1, $cc_email_1);
    ...
    public static function ticket_ackn_emails($email_array)
    {
        ...
        $Subject = 'TMS ticket received. Reference : '.$email_array['ticket_id'];
        $data['users'] = $email_array;
       // $Subject = $email_array['Subject'];
        $team_name = 'Compuscript Team';
        $from_email = 'tmscompuscript@gmail.com';
    ...
    public static function getAll_Leaves_bymonth($user_id,$role,$group_email)
    {
    ...
    $leave_Details_query->where('user_leavelist.leave_status','!=','Reject');
    $leave_Details_query->where('user_leavelist.leave_status','!=','Withdraw');
    
     $leave_Details_query->where(function ($query) use ($current_year) {
                        $query->where('user_leavelist.created_date', 'like', '%' .$current_year. '%' )
                              ->orWhere('user_leavelist.from_date', 'like', '%' .$current_year. '%' )
                              ->orWhere('user_leavelist.to_date', 'like', '%' .$current_year. '%' );
                     });

   
   // $leave_Details_query->where('user_leavelist.from_date', 'like', '%' .$current_month. '%' );
   //$leave_Details_query->whereMonth('user_leavelist.from_date', $current_month);
  
    $leave_Details_query->orderBy('user_leavelist.leave_status', 'asc');
    $leave_Details_query->orderBy('user_leavelist.from_date', 'asc');
...
/* withdraw mail notification */

public static function withdraw_common_emails($email_array)
{
    $data['users'] = $email_array;
    $Subject = $email_array['Subject'];
    $team_name = 'TMS';
    ...
    $cc_email = 'l.osullivan@compuscript.com';

   if(strcmp($email_array['datas']['data-leave_status'],"Approved")==0) {
    $cc_email = 'l.osullivan@compuscript.com';
   } else {
    $cc_email = 'm.lyons@compuscript.com';
...
public static function getAll_bank_bymonth()
{
    $current_year = date('Y'); //calender show by Venkat
    $current_month = date('m');
   /*  $bank_holidays = DB::table('holidays')->select('*')
    ->where('holidays.date', 'like', '%' .$current_year. '%' )->get()->toArray(); */
    $bank_holidays = DB::table('holidays')->select('*')
    ->get()->toArray();
...
public static function getAll_Leaves_bymonth_ajax($user_id,$role,$group_email,$month,$year)
    {
    ...
    $leave_Details_query->where('user_leavelist.leave_status','!=','Reject');
    $leave_Details_query->where('user_leavelist.leave_status','!=','Withdraw');
   // $leave_Details_query->whereYear('user_leavelist.created_date', $current_year);
    ...
    $leave_Details_query->whereMonth('user_leavelist.to_date', $current_month);
   // $leave_Details_query->where('user_leavelist.from_date', 'like', '%' .$current_month. '%' );
   // $leave_Details_query->where('user_leavelist.month(from_date)', '=',$current_month);

  //  $leave_Details_query->where('month(from_date)', '=', $current_month);
...
exit; */
    ...
    public static function get_break_time($user_id) {

        $today_date = date('Y-m-d');


        $break_time = DB::table('break_time')->select('*')
        ->where('user_id', '=', $user_id)
        ->where('break_date', '=', $today_date)
        ->where('break_in', '=', '00:00:00')
        ->orderBy('id','DESC')
        ->skip(0)->take(1)        
        ->get()->count();
        return $break_time;
    ...
    public static function getUserId($accept_v) {


        $user_ids = DB::table('user_leavelist')->select('*')
        ->where('leave_id', '=', $accept_v)  
        ->first();
         return $user_ids;
    ...
    /* Get Manager details by email id */

    public static function getManager_byemail($grp_email) {

        $user_ids = DB::table('users')->select('*')
        ->where('email', '=', $grp_email)  
        ->first();
         return $user_ids;
    ...
    public static function getAll_EmployeeAdmin()
    {
    ...
    public static function getAll_LeavesAdmin()
    {
    ...
    public static function getAll_bank_bymonth_table($year,$month)
{
    //$current_year = date('Y');
    $current_year = $year;
    $current_month = $month;
    $bank_holidays = DB::table('holidays')->select('*')
    ->whereMonth('holidays.date', $current_month)
    ->where('holidays.date', 'like', '%' .$current_year. '%' )->get()->toArray();
    return $bank_holidays;
}
  

public static function get_bank_holidays_inbetweendays($start_date,$end_date){

   

    $bank_holidays_betweendays = DB::table('holidays')
            ->select('*')            
            ->whereBetween('holidays.date', [$start_date, $end_date])            
            ->count();   

     return $bank_holidays_betweendays;        
